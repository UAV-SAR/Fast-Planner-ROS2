...
void SDFMap::initMap(rclcpp::Node::SharedPtr nh) {
  ...
  node_->get_parameter("sdf_map/local_update_range_x", mp_.local_update_range_(0));
  ...
  node_->get_parameter("sdf_map/depth_filter_mindist", mp_.depth_filter_mindist_);
  ...
  node_->get_parameter("sdf_map/p_max", mp_.p_max_);
  ...
  node_->get_parameter("sdf_map/ground_height", mp_.ground_height_);
  ...
  if (mp_.pose_type_ == POSE_STAMPED) {
    ...
    sync_image_pose_ = std::make_shared<message_filters::Synchronizer<SyncPolicyImagePose>>(SyncPolicyImagePose(100), depth_sub_, pose_sub_);
    ...
    sync_image_odom_ = std::make_shared<message_filters::Synchronizer<SyncPolicyImageOdom>>(SyncPolicyImageOdom(100), depth_sub_, odom_sub_);
    sync_image_odom_->registerCallback(std::bind(&SDFMap::depthOdomCallback, this, std::placeholders::_1, std::placeholders::_2));
  }

  // use odometry and point cloud

  indep_cloud_sub_ = node_->create_subscription<sensor_msgs::msg::PointCloud2>("/sdf_map/cloud", 10, std::bind(&SDFMap::cloudCallback, this, std::placeholders::_1));
  indep_odom_sub_ = node_->create_subscription<nav_msgs::msg::Odometry>("/sdf_map/odom", 10, std::bind(&SDFMap::odomCallback, this, std::placeholders::_1));

  occ_timer_ = node_->create_wall_timer(std::chrono::duration<double>(0.05), std::bind(&SDFMap::updateOccupancyCallback, this));
  esdf_timer_ = node_->create_wall_timer(std::chrono::duration<double>(0.05), std::bind(&SDFMap::updateESDFCallback, this));
  vis_timer_ = node_->create_wall_timer(std::chrono::duration<double>(0.05), std::bind(&SDFMap::visCallback, this));
...
void SDFMap::odomCallback(const nav_msgs::msg::Odometry &odom) {
...
void SDFMap::cloudCallback(const sensor_msgs::msg::PointCloud2 &img) {
...
void SDFMap::depthCallback(const sensor_msgs::msg::Image &img) {
  std::cout << "depth: " << rclcpp::Time(img.header.stamp).seconds() << std::endl;
}

void SDFMap::poseCallback(const geometry_msgs::msg::PoseStamped &pose) {
  std::cout << "pose: " << rclcpp::Time(pose.header.stamp).seconds() << std::endl;

...
bool FastPlannerManager::kinodynamicReplan(Eigen::Vector3d start_pt, Eigen::Vector3d start_vel,
  ...
  if (tn / to > 3.0) RCLCPP_ERROR(rclcpp::get_logger("planner_manager"), "reallocate error.");
...
bool FastPlannerManager::planGlobalTraj(const Eigen::Vector3d& start_pos) {
  ...
  RCLCPP_INFO(rclcpp::get_logger("planner_manager"), "global trajectory generated.");
...
bool FastPlannerManager::topoReplan(bool collide) {
  ...
  if (!collide) {  // simply truncate the segment and do nothing
    ...
    if (colli_start.size() == 1 && colli_end.size() == 0) {
      RCLCPP_WARN(rclcpp::get_logger("planner_manager"), "Init traj ends in obstacle, no replanning.");
      local_data_.position_traj_ = init_traj;
      ...
      RCLCPP_INFO(rclcpp::get_logger("planner_manager"), "[Topo]: ---------");
      plan_data_.clearTopoPaths();
...
void FastPlannerManager::refineTraj(NonUniformBspline& best_traj, double& time_inc) {
  ...
  RCLCPP_WARN_STREAM(
    ...
    "[Refine]: cost " << (rclcpp::Clock().now() - t1).seconds()
...
void FastPlannerManager::optimizeTopoBspline(double start_t, double duration,
  ...
  if (guide_pt.size() != int(ctrl_pts.rows()) - 6) RCLCPP_WARN(rclcpp::get_logger("planner_manager"), "what guide");

  tm1 = (rclcpp::Clock().now() - t1).seconds();
  t1  = rclcpp::Clock().now();
  ...
  tm2 = (rclcpp::Clock().now() - t1).seconds();
  t1  = rclcpp::Clock().now();
  ...
  tm3 = (rclcpp::Clock().now() - t1).seconds();
  RCLCPP_INFO(rclcpp::get_logger("planner_manager"), "optimization %d cost %lf, %lf, %lf seconds.", traj_id, tm1, tm2, tm3);
}
...
void FastPlannerManager::planYaw(const Eigen::Vector3d& start_yaw) {
  RCLCPP_INFO(rclcpp::get_logger("planner_manager"), "plan yaw");

...
rclcpp::Publisher<visualization_msgs::msg::Marker>::SharedPtr state_pub, traj_pub;
rclcpp::Publisher<swarmtal_msgs::msg::DroneOnboardCommand>::SharedPtr pos_cmd_pub;
...
int main(int argc, char** argv) {
  ...
  auto node = std::make_shared<rclcpp::Node>("traj_generator");

  auto odom_sub = node->create_subscription<nav_msgs::msg::Odometry>(
      "/uwb_vicon_odom", 50, odomCallback);

  traj_pub = node->create_publisher<visualization_msgs::msg::Marker>(
      "/traj_generator/traj_vis", 10);
  state_pub = node->create_publisher<visualization_msgs::msg::Marker>(
      "/traj_generator/cmd_vis", 10);

  pos_cmd_pub = node->create_publisher<swarmtal_msgs::msg::DroneOnboardCommand>(

...
from tf_transformations import euler_from_quaternion  # Replaces tf.transformations


class OdomToEulerNode(Node):
    def __init__(self):
        ...
        self.sub1 = self.create_subscription(Imu, 'imu', self.imu_callback, 10)
        self.sub2 = self.create_subscription(Joy, 'ctrlin', self.joy_callback, 10)
    ...
    def odom_callback(self, odom_msg):
        ...
        euler_msg.vector.z = e[0] * 180.0 / np.pi
        euler_msg.vector.y = e[1] * 180.0 / np.pi
        euler_msg.vector.x = e[2] * 180.0 / np.pi
